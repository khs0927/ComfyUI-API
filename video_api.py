...
app = FastAPI(
    ...
    version="0.1.0",
    description=(
        "Scene-based AI video orchestration for ChatGPT, ComfyUI, Hugging Face Spaces, and fal.ai. "
        "The service has no fixed final-duration limit; provider quotas and available compute still apply."
    ),
)
app.add_middleware(
    CORSMiddleware,
...
async def health() -> dict[str, object]:
    return {
        ...
        "providers": {
            "comfyui": bool(os.getenv("COMFYUI_URL") and os.getenv("COMFYUI_VIDEO_WORKFLOW")),
            "fal_ltx": bool(os.getenv("FAL_KEY")),
            "hf_space": bool(os.getenv("HF_VIDEO_SPACE_ID")),
            "mock": True,
        },
    }
...
async def create_video(request: VideoRequest, authorization: str | None = Header(default=None)) -> dict[str, object]:
...
@app.get("/v1/videos")
async def list_videos(authorization: str | None = Header(default=None)) -> list[dict[str, object]]:
    authorize(authorization)
    return [asdict(job) for job in await engine.store.list()]


@app.get("/v1/videos/{job_id}")
async def get_video(job_id: str, authorization: str | None = Header(default=None)) -> dict[str, object]:
    authorize(authorization)
...
async def cancel_video(job_id: str, authorization: str | None = Header(default=None)) -> dict[str, object]:
    authorize(authorization)
...
async def download_video(job_id: str, authorization: str | None = Header(default=None)) -> FileResponse:
    authorize(authorization)
    job = await engine.get(job_id)
